...
CurrentWeatherLayer* current_weather_layer_create_layer(GRect frame, CurrentWeather current_weather) {
  ...
  *weather_layer = (CurrentWeatherLayer) {
    ...
    .temperature_layer = copying_text_layer_create(GRect(8, 50, 45, 35), "X", 7),
  ...
  copying_text_layer_set_text_color(weather_layer->temperature_layer, GColorWhite);
  copying_text_layer_set_background_color(weather_layer->temperature_layer, GColorClear);
  copying_text_layer_set_font(weather_layer->temperature_layer, fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD));
  copying_text_layer_set_text_alignment(weather_layer->temperature_layer, GTextAlignmentCenter);
  
  layer_add_child(layer, copying_text_layer_get_layer(weather_layer->temperature_layer));
